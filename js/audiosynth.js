...
!function(){
	...
	setPub('play', function(sound, note, octave, duration) {
		console.log(159, sound, note, octave, duration)
		var src = this.generate(sound, note, octave, duration);
		var audio = new Audio(src);
		audio.play();
		if(keytype<10){
			var c = ["#059862", "#dd4a68", "#9a6e3a", "#3a3e9a", "#ead30d", "#d80dea", "#FFFFFF", "#000000", "#FF0000", "#00FF00"];
			var p = ["right", "left", "center"];
			var i = [
				"https://www.w3schools.com/jsref/img_tree.png", 
				"https://upload.wikimedia.org/wikipedia/en/f/f6/Tom_Tom_and_Jerry.png", 
				"https://icon2.cleanpng.com/20171220/dae/farm-landscape-vector-illustration-5a3b32f72ddb67.1491796715138291111878.jpg", 
				"https://i0.wp.com/www.careandlearningalliance.co.uk/wp-content/uploads/2018/12/christmas-tree-gif.gif?fit=239%2C373&ssl=1", 
				"https://e7.pngegg.com/pngimages/402/930/png-clipart-home-animal-cartoon-illustration-cartoon-mountain-farm-farm-cartoon-character-child-thumbnail.png"
			];
			document.body.style.background = c[Math.floor(Math.random() * c.length)] + " url('"+ i[Math.floor(Math.random() * i.length)] +"') no-repeat "+ p[Math.floor(Math.random() * p.length)] +" top";
		}
		return true;
	});
